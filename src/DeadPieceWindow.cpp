#include "../include/DeadPieceWindow.h"
...
DeadPieceWindow::DeadPieceWindow(const wxString &title)
  ...
  wxPanel *whitePanel =
      new wxPanel(this, wxID_ANY, wxDefaultPosition, wxSize(w, -1));
  // Create the listview
  ...
  // Header text
  wxStaticText *whiteDead = new wxStaticText(
  ...
  // Header text
  wxStaticText *blackDead = new wxStaticText(
...
void DeadPieceWindow::addWhite(const wxString &id, const int &cellX,
                               const int &cellY) {
...
void DeadPieceWindow::addBlack(const wxString &id, const int &cellX,
                               const int &cellY) {
...
void DeadPieceWindow::removeFirstWhite() { m_whiteList->DeleteItem(0); }

void DeadPieceWindow::removeFirstBlack() { m_blackList->DeleteItem(0); }
